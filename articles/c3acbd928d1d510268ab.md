...
- getterによりビジネスロジックがオブジェクト外に流出するのは悪。

# 想定読者

- オブジェクト指向に入門した方々
- オブジェクト指向の理念とかは理解したけど、具体的な実装を知りたい方々
- というか1年前の僕
...
なので普通の人なら「getter/setterはアンチパターンなのだ」と認識することでしょう。
...
getterやsetterを使うな、と言われても入門者にとっては「じゃあどうすれば良いの？」という感じだと思います。

少なくとも1年前の僕は困惑しました。 そんな方々に届けばと思って書きました。
本記事ではPHPによる実装を通じて、悪いgetter/setterを表現し、それがなぜ悪いのか、どう改善すれば良いのかを説明したいと思います。
...
この2つを同列で扱って議論しているから、いつまでも議論が終着しないのだと思います。
...
一方でgetterはインターフェース層での出力やDBへの値変換の際に、必要なケースがあります。
**getterは生やさなくていけないケースがある**ので、これも一律で「不要」「悪」と断じることはできません[^1]。
ただし、getterは使い方を間違えると「悪」となるケースがあり、それは**ビジネスロジックがオブジェクトの外に流出しているケース**です。
つまり、getterには悪となるケースとそうでないケースがあり、getterで括って善悪を判断することはできないと思われます。
...
同じ処理を書いてしまったということは、ビジネスロジックが各地に分散してしまったことになります。
後日このビジネスロジックの仕様を確認したいときに、おそらく`calc_total_price`関数と`is_over`関数の2つを追う必要に迫られるでしょう。
...
`ItemOrder`クラスがビジネスロジックを持つことにより、データが使われるビジネスロジックがどこなのか、ビジネスロジックのインプットとなるデータはどれなのかが追いやすくなります。
`calc_total_price`関数にビジネスロジック、`ItemOrder`クラスにデータ、というように分離しているわけでもないので、再利用性も高まります[^8]。

つまり、データとビジネスロジックは密結合にしておくべきだったのに、**getterによりビジネスロジックをデータを持っているオブジェクトの外へと流出させてしまったのが問題**でした。
...
getterを生やした上で、レビュー等でビジネスロジックをオブジェクト外に流出させていないかを指摘し合うという運用でカバーする方針になるかと思います。
...
- オブジェクト生成ができなかったときのハンドリングは使用側に任せる(この場合`catch`して`return 0;`させています)

この実装のメリットとして、以下の3つがあります。

- setterによってオブジェクトが破壊される恐れがなくなった。
- バリデーションメソッドが不要になった。正確にはオブジェクトが生成できているというのがバリデーションに通ってることを意味するようになった。
...
- 合計額の計算のビジネスロジックをgetterを使わず`ItemOrder`クラスに持たせた。
- ifの分岐をprivateメソッドとして命名することで、どういうビジネスロジックなのかをコードとして明文化した[^12]。

これによって後日ビジネスロジックの仕様を調べたいときには追いやすくなり、再利用性も高まっているはずです。

### 追加の関数により再利用性が向上したことの確認
...
- オブジェクト指向をちゃんと勉強したい場合は「現場で役立つシステム設計の原則」を読めば間違いないです。
- 例えばRailsのActiveRecordはこの記事の思想とは大きく反していると思います。やっぱり現場によるので、柔軟に対応しましょう。
- ドヤ顔で解説記事を書きましたが、筆者もオブジェクト指向にそこまで自信があるわけではないです。コメント・マサカリを歓迎しております。
...
[^8]: ドヤ顔で解説していますが、「現場で役立つシステム設計の原則」に同じことが書いてあります。全人類、この本を読みましょう。
...
[^12]: 数字の定数化は面倒だった。いやこの記事の趣旨に対してオーバーエンジリアリングです(?)。
